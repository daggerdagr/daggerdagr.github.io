...
var half_container = ($('.container').height() / 2)
...
$('.container').waypoint(function(direction) {
  ...
  }} , {offset: half_container + menu_height + 10})

//parseInt(half_container + menu_height + 100
...
$('.container').waypoint(function(direction) {
	...
	if (direction === 'up'){
	  	menu_activate(curr_page)
	    console.log(curr_page)
  }} , {offset: ((half_container) * -1) + menu_height + 10});
//(half_container * -1) + menu_height - ($(window).height() * 0.01)
/*$('.container').waypoint(function(direction) {
	var curr_page = $(this).attr('element').id
...
$('.menu ul li').click(function() {
	...
	if (corresponding_pg == 'resume') {
		$('html, body').animate({
	      scrollTop: $('.container#'+corresponding_pg).offset().top - ($('.container').height() / 2) - menu_height
	    }, 1500, 'easeInOutCubic')	
	} else {
		console.log($('.container#'+corresponding_pg).offset().top)
		$('html, body').animate({
	      scrollTop: $('.container#'+corresponding_pg).offset().top - menu_height
	    }, 1500, 'easeInOutCubic');
	}
	...
	//menu_activate(corresponding_pg)
});

$('.proj-item').click(function() {
	$('.proj-item-window').addClass('active');
	$('.proj-item-window').css('top', $(window).scrollTop() + menu_height)
